...
--1. Display Products with Average Sales Amount Between 1000 and 2000, Ordered by Product Name
	select product,avg(sales_amount) from sales_data group by product having avg(sales_amount) between 1000 and 2000

--2. Display Years with More Than 5 Orders from Each Region
	select year,COUNT(product),region  from sales_data group by region,year having COUNT(product)  >5

--3. Display Regions with Average Sales Amount Above 1500 in Year 2023 sort by amount in descending.
	select region,avg(sales_amount) from sales_data where year=2023 group by region having avg(Sales_Amount) > 1500
